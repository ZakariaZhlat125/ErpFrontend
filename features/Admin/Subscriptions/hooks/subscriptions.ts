...
export const useSubscribeUser = () => {
  ...
  return useMutation({
    mutationFn: (data: SubscribeUserInput) => subscriptionsApi.subscribeUser(data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [QUERY_KEY] });
    },
  });
...
export const useChangeSubscriptionStatus = () => {
  ...
  return useMutation({
    mutationFn: ({ id, data }: { id: string; data: ChangeStatusInput }) => 
      subscriptionsApi.changeStatus(id, data),
    onSuccess: (_, { id }) => {
      queryClient.invalidateQueries({ queryKey: [QUERY_KEY] });
      queryClient.invalidateQueries({ queryKey: [QUERY_KEY, id] });
    },
...
export const useRenewSubscription = () => {
  ...
  return useMutation({
    mutationFn: ({ id, data }: { id: string; data?: RenewSubscriptionInput }) => 
      subscriptionsApi.renewSubscription(id, data),
    onSuccess: (_, { id }) => {
      queryClient.invalidateQueries({ queryKey: [QUERY_KEY] });
      queryClient.invalidateQueries({ queryKey: [QUERY_KEY, id] });
    },
...
export const useCancelSubscription = () => {
  ...
  return useMutation({
    mutationFn: ({ id, data }: { id: string; data?: CancelSubscriptionInput }) => 
