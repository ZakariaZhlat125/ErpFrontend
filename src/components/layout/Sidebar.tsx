...
export function Sidebar({ collapsed = false, onToggle }: SidebarProps) {
  ...
  const { groupedItems, orderedCategories, standaloneItems } = useMemo(() => {
    ...
    return { groupedItems: groups, orderedCategories: ordered, standaloneItems: standalone };
  }, [menuItems]);
  ...
  return (
    ...
    >
      ...
      <nav className={`flex-1 space-y-2 overflow-y-auto scrollbar-thin scrollbar-thumb-white/20 ${collapsed && !isHovered ? 'px-2 py-4' : 'px-4 py-4'}`}>
        {/* Standalone items (Admin menu items without categories) */}
        ...
        {/* Categorized items (Organization menu items) */}
